(() => {
  ...
  angular
  .module('sonnyProject')
  .controller('landingoneController', landingoneController);

  landingoneController.$inject = ['$stateParams', '$state', 'servicioUsuarios'];

  function landingoneController($stateParams, $state, servicioUsuarios){
    let vm = this;
    ...
    vm.registrarVehiculo = (pusuario) => {
      ...
      $state.go('cars', { objUsuarioTemp : JSON.stringify(pusuario)});
    }
